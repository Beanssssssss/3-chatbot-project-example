...
(() => { // IIFE 시작
    ...
    async function sendMessage() {
        ...
        try {
            ...
            // botData 추출: outer.reply가 있으면 파싱, 없으면 outer 자체를 사용
            let botData = null;
            if (outer.reply) {
                try {
                    // reply가 JSON 문자열인 경우 파싱
                    botData = typeof outer.reply === 'string' ? JSON.parse(outer.reply) : outer.reply;
                } catch (e) {
                    console.error('Failed to parse inner JSON:', e);
                    console.error('Received reply:', outer.reply);
                    appendMessage('bot', '챗봇 응답 형식 오류.');
                    conversationHistory.pop();
                    return;
                }
            } else if (outer.bot_message || outer.is_analysis !== undefined) {
                // outer 자체가 botData 형식인 경우 (서버가 직접 객체 반환)
                botData = outer;
            }

            if (botData) {
                if (botData.error) {
                    console.error('Backend logic error:', botData.error);
                    appendMessage('bot', botData.error);
                 ...
                 console.error('API level error:', outer.error);
                 appendMessage('bot', outer.error);
                 conversationHistory.pop(); // 오류 시 사용자 메시지 제거
            } else {
                 console.error('Unknown response format:', outer);
                 appendMessage('bot', '알 수 없는 응답 형식.');
                 conversationHistory.pop(); // 오류 시 사용자 메시지 제거
            }
